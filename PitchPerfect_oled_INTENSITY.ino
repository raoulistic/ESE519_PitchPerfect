...
int Gyr_Threshold = 180;
int windowSize = 230;

String filename;
int filenum=0;

float maxdiff1 = 0, maxdiff2 = 0;
int logdiff1 = 0, logdiff2 = 0;
  ...
  */
  ...
  /*delay(2000);
    ...
    while(true){
      ...
      filename="Sample_"+String(filenum)+".txt";
...
void loop() {
  ...
  if (countprint == 100) {
    ...
    display.setCursor(0, 0);
    display.println("PitchPerfect!");
    display.setCursor(0, 15);
    display.println("Pitch Detection Band");
    display.display();
    //countprint = 0;
  }
  countprint += 1;
  ...
  if (pitch_count >= 3) {
    ...
    if (data_pts > (logdiff1 + windowSize))
    {
      maxdiff1 = 0;
    }
    if (diff2 > maxdiff2)
  ...
  if (data_pts > (logdiff2 + windowSize))
  {
    maxdiff2 = 0;
  }


  static bool maxprinted = false;
  float diff1pct = maxdiff1/61;
  float diff2pct = maxdiff2/275;
  float totalpct = (diff1pct + diff2pct)/2;
  static float totalpct_prev=0;
  /*if(totalpct!=totalpct_prev){
    maxprinted=false;
    totalpct_prev=totalpct;
    }
  */
  Serial.print("max diffs: ");
  Serial.println(String(maxdiff1)+" "+String(maxdiff2));
  ...
  if(!maxprinted){
    if(totalpct>0.3){
       display.display();
        ...
        display.setCursor(0, 0);
        String str1, str2;
        str1 = "Pitch Count = " + String(pitch_count)+"  "+String(filenum);
        display.println(str1);
        //display.setCursor(, 0);
        display.setCursor(0, 10);
        
        str2 = "Rest Days:"+String(restDays);
        display.println(str2);
        display.setCursor(0, 20);
        display.println("Intensity: "+String(totalpct*100)+"%");
        display.display();
        maxprinted =true;
